...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,addmentions,gapscan,callback) {
    ...
    // for (var cindx = 0; cindx < contexts.length; ++ cindx) {
        ...
        //Build context query
        // TODO add logic to remove extra {timestamp} connections between context and user
        // TODO also make context names shorter like c_1, c_2 etc.

        createContexts += 'MERGE (' + 'c_' + '0' + ':Context ' + '{name:"' + contexts[0].name + '",by:u.uid,uid:"'+contexts[0].uid+'"}) ON CREATE SET ' + 'c_' + '0' + '.timestamp=$timestamp MERGE ' + '(c_' + '0' + ')' + '-[:BY{timestamp:$timestamp}]->(u) ';
    // }

    // Create the statement

    // if (concepts.length > 0) {
        createStatement += 'CREATE (s:Statement ' + '{name:$statementName,';
    // }
    // else {
    ...
    params['statementName'] += '#' + concepts[0];

    params['concepts'].push(concepts[0]);
      ...
      for (var index = 1; index < concepts.length; ++ index) {
          ...
          if (concepts_added.indexOf(concepts[index]) == -1) {
              ...
              params['concepts'].push(concepts[index]);
    ...
    createNodesQuery += ' FOREACH (conceptName in $concepts | ';
    createNodesQuery += ' MERGE (concept:Concept ' + '{name:conceptName}) ON CREATE SET concept.uid=apoc.create.uuid() ';
    ...
    if (typeof mentions !== 'undefined' && mentions.length > 0) {
        ...
        for (var puka = 0; puka < mentions.length; puka++) {
            ...
            if (mentions_added.indexOf(mentions[puka]) == -1) {
                ...
                var current_mention = mentions[puka];
                if (addmentions == 'remove' || addmentions == 'link') {
                  current_mention = current_mention.substring(1);
                }

                // Add the node we just added into another array, to check for duplicates in the next cycle
                mentions_added.push(mentions[puka]);

                createFirstMentionQuery += 'MERGE (' + convertMention(mentions[puka], mentions_added) + ':Concept ' + '{name:"' + current_mention + '"}) ON CREATE SET ' + convertMention(mentions[puka], mentions_added)  + '.uid="' + ment_uid + '" ';
    ...
    if (concepts.length > 0) {
        ...
        // for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            // Link to user
            createNodesQuery += 'CREATE (concept)-[:BY {context:c_' + '0' + '.uid,timestamp:$timestamp,statement:s.uid}]->(u) ';

            // Link to context
            createNodesQuery += 'CREATE (concept)-[:OF {context:c_' + '0' + '.uid,user:u.uid,timestamp:$timestamp}]->(s)  CREATE (concept)-[:AT {user:u.uid,timestamp:$timestamp,context:c_' + '0' + '.uid,statement:s.uid}]->('+'c_' + '0' + ') ';
            ...
            if (typeof mentions !== 'undefined' && mentions.length > 0) {
                for (var index = 0; index < mentions.length; index++) {
                    ...
                    createMentionsQuery += 'CREATE (' + convertName(concepts[0], concepts_added) +')-[:TO {context:c_' + indx + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edgement_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[index], mentions_added) + ') ';
                }
            }
        ...
        createNodesQuery += ' WITH u';

        // for (var cindx = 0; cindx < contexts.length; ++ cindx) {

                    createNodesQuery += ', c_' + '0';
        // }
    ...
    if (concepts.length == 1) {
        ...
        if (mentions.length == 0) {
            ...
            for (var nindx = 0; nindx < contexts.length; ++ nindx) {
              ...
              //TODO  createEdgesQuery += 'CREATE (' + convertName(concepts[0], concepts_added) +')-[:TO {context:c_' + nindx + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->(' + convertName(concepts[0], concepts_added) + ') ';
    ...
    if (mentions.length == 1) {
        ...
        if (concepts.length == 0) {
            ...
            for (var mindx = 0; mindx < contexts.length; ++ mindx) {
                ...
                createMentionsQuery += 'CREATE (' + convertMention(mentions[0], mentions_added) +')-[:TO {context:c_' + mindx + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + ment_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[0], mentions_added) + ') ';
        ...
        if (concepts[minusOne] !== concepts[index]) {
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
              ...
              if ((mentions.length == 0) || (mentions.length > 0 && addmentions != 'link')) {
                ...
                params['conceptsRelations'].push({from:concepts[minusOne],to:concepts[index],context:contexts[indx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"2",weight:narrativeScanWeight});

                //createEdgesQuery += 'CREATE (' + convertName(concepts[minusOne], concepts_added) +')-[:TO {context:c_' + indx + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->(' + convertName(concepts[index], concepts_added) + ') ';
                ...
                if (typeof mentions !== 'undefined' && mentions.length > 0) {
                    for (var m = 0; m < mentions.length; m++) {
                        edgement_uid = uuid.v1();
                        createMentionsQuery += 'CREATE (' + convertName(concepts[index], concepts_added) +')-[:TO {context:c_' + indx + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edgement_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[m], mentions_added) + ') ';
                    }
                }
            ...
            for (var indexGap = leftGap; indexGap < (index - 1); ++indexGap) {
                ...
                if (concepts[indexGap] !== concepts[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                        ...
                        // Make query
                        params['conceptsRelations'].push({from:concepts[indexGap],to:concepts[index],context:contexts[indx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:scanGap,weight:weightInGap});

//                        createEdgesQuery += 'CREATE (' + convertName(concepts[indexGap], concepts_added) +')-[:TO {context:c_' + indx + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"' + scanGap + '",weight:"' + weightInGap + '"}]->(' + convertName(concepts[index], concepts_added) + ') ';
        ...
        // Let's link that concept to Statement, Context and User even if there was a similar one before (can use it later for Weight)
            ...
            params['statementName'] += ' #' + concepts[index];
    }
    ...
    if (mentions.length > 0) {
        ...
        for (var index = 0; index < mentions.length; index++) {
            ...
            for (var iter = 0; iter < index; iter++) {
                ...
                if (mentions[iter] !== mentions[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                        ...
                        createMentionsQuery += 'CREATE (' + convertMention(mentions[iter], mentions_added) +')-[:TO {context:c_' + indx + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[index], mentions_added) + ') ';
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
                createMentionsQuery += 'CREATE (' + convertMention(mentions[index], mentions_added)  + ')-[:BY {context:c_' + indx + '.uid,timestamp:"' +(timestamp+index)+ '",statement:s.uid}]->(u) ';
                createMentionsQuery += 'CREATE (' + convertMention(mentions[index], mentions_added) + ')-[:OF {context:c_' + indx + '.uid,user:u.uid,timestamp:"'+(timestamp+index)+'"}]->(s) CREATE (' + convertMention(mentions[index], mentions_added) + ')-[:AT {user:u.uid,timestamp:"' + (timestamp+index) + '",context:c_' + indx + '.uid,statement:s.uid}]->('+ 'c_' + indx +') ';
            }

            params['statementName'] += ' ' + mentions[index];

        }
    }

    createStatement += ' text:$statementText, uid:"' + statement.uid + '", timestamp:"' + timestamp + '"}) ';

    for (var indx = 0; indx < contexts.length; ++ indx) {
        createStatement += 'CREATE (s)-[:BY {context:c_' + indx + '.uid,timestamp:s.timestamp}]->(u) ';
        createStatement += 'CREATE (s)-[:IN {user:u.id,timestamp:s.timestamp}]->('+'c_' + indx + ') ';
    }

    createEdgesQuery += ' UNWIND $conceptsRelations as conceptsRelation ';
    // for (var indx = 0; indx < contexts.length; ++ indx) {
        createEdgesQuery += 'MATCH (c_from:Concept{name: conceptsRelation.from}) MATCH (c_to:Concept{name: conceptsRelation.to}) ';
        createEdgesQuery += 'CREATE (c_from)-[:TO {context:c_' + '0' + '.uid,statement:s.uid,user:u.uid,timestamp:$timestamp, uid:apoc.create.uuid(), gapscan:conceptsRelation.gapscan, weight: conceptsRelation.weight}]->(c_to) ';
    // }

    createNodesEdgesQuery += matchUser + createContexts + createStatement + createFirstMentionQuery + createNodesQuery + createEdgesQuery + createMentionsQuery + ' RETURN DISTINCT s.uid ';
