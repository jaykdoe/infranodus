...
CypherQuery.addStatement = function(user, statements, contexts, addmentions, gapscan, callback) {
    ...
    var mentions_exist = 0;
    var concepts_exist = 0;
    ...
    createContexts += 'ON CREATE SET context.timestamp=$timestamp MERGE (context)-[:BY{timestamp:$timestamp}]->(u) ';

    // Create the statement

    createStatement += 'WITH u, context ';
              ...
              for (var index = 0; index < concepts.length; index++) {
                ...
                if (index > 0) {
                    ...
                    if (concepts[minusOne] !== concepts[index]) {
                        ...
                        for (var indx = 0; indx < contexts.length; ++ indx) {
                          ...
                          if ((mentions.length == 0) || (mentions.length > 0 && addmentions != 'link')) {
                            ...
                            params['conceptsRelations'].push({from:concepts[minusOne],to:concepts[index],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"2",weight:narrativeScanWeight});
                          ...
                          if (typeof mentions !== 'undefined' && mentions.length > 0) {
                                ...
                                for (var m = 0; m < mentions.length; m++) {
                                    ...
                                    params['mentionsRelations'].push({from:concepts[index],to:mentions[m],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                        ...
                        for (var indexGap = leftGap; indexGap < (index - 1); ++indexGap) {
                            ...
                            if (concepts[indexGap] !== concepts[index]) {
                                ...
                                for (var indx = 0; indx < contexts.length; ++ indx) {
                                    ...
                                    // Make query
                                    params['conceptsRelations'].push({from:concepts[indexGap],to:concepts[index],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:scanGap,weight:weightInGap});
                  ...
                  if (concepts_added.indexOf(concepts[index]) == -1) {
                      ...
                      params['statements'][sindex]['uniqueconcepts'].push({name:concepts[index],suid:params['statements'][sindex]['uid'],timestamp:params['statements'][sindex]['timestamp']});

                  }

                  // TODO remove this as it is unnecessary
                  // // It exists? Remove it from the common list // TODO Maybe unnecessary
                  // else {
                  //
                  //     params['statements'][sindex]['concepts'].splice(index, 1);
                  //
                  //     if (index != (params['statements'][sindex]['concepts'].length - 2)) {
                  //       index = index + 1;
                  //     }
                  // }
        ...
        if (params['statements'][sindex]['mentions']) {
                      ...
                      for (var puka = 0; puka < params['statements'][sindex]['mentions'].length; puka++) {
                          ...
                          for (var iter = 0; iter < puka; iter++) {
                              ...
                              if (params['statements'][sindex]['mentions'][iter] !== params['statements'][sindex]['mentions'][puka]) {
                                  ...
                                  for (var indx = 0; indx < contexts.length; ++ indx) {
                                        ...
                                        params['mentionsRelations'].push({from:params['statements'][sindex]['mentions'][iter],to:params['statements'][sindex]['mentions'][puka],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                          ...
                          if (mentions_added.indexOf(params['statements'][sindex]['mentions'][puka]) == -1) {
                              ...
                              params['statements'][sindex]['uniquementions'].push({name:params['statements'][sindex]['mentions'][puka],suid:params['statements'][sindex]['uid'],timestamp:params['statements'][sindex]['timestamp']});

                              if (params['statements'][sindex]['concepts'] && params['statements'][sindex]['concepts'].length > 0) {
                                for (var indx = 0; indx < contexts.length; indx++) {
                                  params['mentionsRelations'].push({from:params['statements'][sindex]['concepts'][0],to:params['statements'][sindex]['mentions'][puka],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                                }
                              }
    ...
    // if (mentions_exist > 0) {
    //   createMentionsQuery += ' WITH u, s, context, statement FOREACH (mentionName in mentions | ';
    //
    ...
    if (concepts_exist > 0) {
        ...
        createNodesQuery += ' WITH u, s, context, statement FOREACH (conceptName in statement.uniqueconcepts | ';
        createNodesQuery += ' MERGE (c:Concept ' + '{name:conceptName.name}) ON CREATE SET c.uid=apoc.create.uuid() ';
    ...
    //           params['conceptsRelations'].push({from:params['concepts'][0]['concept'],to:params['concepts'][0]['concept'],context:contexts[nindx].uid,statement:params['concepts'][0]['suid'],user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"2",weight:narrativeScanWeight});
    //         }
    //     }
    //
    // }
    ...
    //             params['mentionsRelations'].push({from:params['mentions'][0]['mention'],to:params['mentions'][0]['mention'],context:contexts[mindx].uid,statement:params['mentions'][0]['suid'].uid,user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
    //         }
    //     }
    // }
    ...
    // console.log(JSON.stringify(finalCypherQuery['query']));
