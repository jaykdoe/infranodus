...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,addmentions,gapscan,callback) {
    ...
    createContexts += 'MERGE (context:Context ' + '{name:contextName.name,by:u.uid,uid:contextName.uid}) ON CREATE SET context.timestamp=$timestamp MERGE (context)-[:BY{timestamp:$timestamp}]->(u) ';
    ...
    for (var index = 0; index < concepts.length; ++ index) {
          ...
          if (concepts_added.indexOf(concepts[index]) == -1) {
              ...
              // Add the concept to Neo4J params variable
    ...
    if (typeof mentions !== 'undefined' && mentions.length > 0) {
        ...
        createMentionsQuery += ' WITH u, s, context FOREACH (mentionName in $mentions | ';
        ...
        for (var puka = 0; puka < mentions.length; puka++) {
            ...
            if (mentions_added.indexOf(mentions[puka]) == -1) {
                ...
                if (addmentions == 'remove' || addmentions == 'link') {
                  mentions[puka] = mentions[puka].substring(1);
                }
                ...
                // Add the mention to Neo4J params variable
                params['mentions'].push(mentions[puka]);
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            createMentionsQuery += 'CREATE (mention)-[:BY {context:context.uid,timestamp:$timestamp,statement:s.uid}]->(u) ';
            createMentionsQuery += 'CREATE (mention)-[:OF {context:context.uid,user:u.uid,timestamp:$timestamp}]->(s) CREATE (mention)-[:AT {user:u.uid,timestamp:$timestamp,context:context.uid,statement:s.uid}]->(context) ';
        }
    ...
    if (concepts.length > 0) {
        ...
        // Link to user
        createNodesQuery += 'CREATE (concept)-[:BY {context:context.uid,timestamp:$timestamp,statement:s.uid}]->(u) ';

        // Link to context
        createNodesQuery += 'CREATE (concept)-[:OF {context:context.uid,user:u.uid,timestamp:$timestamp}]->(s)  CREATE (concept)-[:AT {user:u.uid,timestamp:$timestamp,context:context.uid,statement:s.uid}]->(context) ';

        // Close the FOREACH nodes cycle
        createNodesQuery += ' ) ';

        // Create a link between the first concept and all the mentions
        if (typeof mentions !== 'undefined' && mentions.length > 0) {
                 for (var index = 0; index < mentions.length; index++) {
                    for (var indx = 0; indx < contexts.length; indx++) {
                      params['mentionsRelations'].push({from:concepts[0],to:mentions[index],context:contexts[indx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                    }
                 }
        }
    ...
    if (concepts.length == 1) {
        ...
        if (mentions.length == 0) {
            ...
            for (var nindx = 0; nindx < contexts.length; ++ nindx) {
              params['conceptsRelations'].push({from:concepts[0],to:concepts[0],context:contexts[nindx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"2",weight:narrativeScanWeight});
            }
        }
    ...
    if (mentions.length == 1) {
        ...
        if (concepts.length == 0) {
            ...
            for (var mindx = 0; mindx < contexts.length; ++ mindx) {
                params['mentionsRelations'].push({from:mentions[0],to:mentions[0],context:contexts[mindx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
            }
        }
    }
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
              ...
              if (typeof mentions !== 'undefined' && mentions.length > 0) {
                    ...
                    for (var m = 0; m < mentions.length; m++) {
                        ...
                        params['mentionsRelations'].push({from:concepts[index],to:mentions[m],context:contexts[indx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
    ...
    // Add relations between all mentions in all the contexts
            ...
            for (var iter = 0; iter < index; iter++) {
                ...
                if (mentions[iter] !== mentions[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                      ...
                      params['mentionsRelations'].push({from:mentions[iter],to:mentions[index],context:contexts[indx].uid,statement:statement.uid,user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
    ...
    if (params['conceptsRelations'].length > 0) {
      createEdgesQuery += ' WITH u, s UNWIND $conceptsRelations as conceptsRelation ';
      createEdgesQuery += 'MATCH (c_from:Concept{name: conceptsRelation.from}) MATCH (c_to:Concept{name: conceptsRelation.to}) ';
      createEdgesQuery += 'CREATE (c_from)-[:TO {context:conceptsRelation.context,statement:s.uid,user:u.uid,timestamp:$timestamp, uid:apoc.create.uuid(), gapscan:conceptsRelation.gapscan, weight: conceptsRelation.weight}]->(c_to) ';
    }

    if (params['mentionsRelations'].length > 0) {
      createMEdgesQuery += ' WITH u, s UNWIND $mentionsRelations as mentionsRelation ';
      createMEdgesQuery += 'MATCH (m_from:Concept{name: mentionsRelation.from}) MATCH (c_to:Concept{name: mentionsRelation.to}) ';
      createMEdgesQuery += 'CREATE (m_from)-[:TO {context:mentionsRelation.context,statement:s.uid,user:u.uid,timestamp:$timestamp, uid:apoc.create.uuid(), gapscan:mentionsRelation.gapscan, weight: mentionsRelation.weight}]->(c_to) ';
    }
