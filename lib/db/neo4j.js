...
CypherQuery.addStatement = function(user, statements, contexts, addmentions, gapscan, callback) {
    ...
    for (var sindex = 0; sindex < params['statements'].length; sindex++) {
        ...
        if (params['statements'][sindex]['mentions']) {
                      ...
                      if (!params['statements'][sindex]['conceptsRelations']) {
                        params['statements'][sindex]['conceptsRelations'] = [];
                      }
                      ...
                      for (var puka = 0; puka < params['statements'][sindex]['mentions'].length; puka++) {
                          ...
                          if (mentions_added.indexOf(params['statements'][sindex]['mentions'][puka]) == -1) {
                              ...
                              if (params['statements'][sindex]['concepts'] && params['statements'][sindex]['concepts'].length > 0) {
                                for (var indx = 0; indx < contexts.length; indx++) {
                                  params['statements'][sindex]['mentionsRelations'].push({from:params['statements'][sindex]['concepts'][0],to:params['statements'][sindex]['mentions'][puka],context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                                }
                              }
    ...
    if (concepts_exist > 0) {
        ...
        createNodesQuery += ' FOREACH (conceptName in statement.uniqueconcept | ';
    ...
    if (concepts_exist > 0) {
      createEdgesQuery += ' WITH u, s, statement UNWIND statement.conceptsRelations as conceptsRelation ';
