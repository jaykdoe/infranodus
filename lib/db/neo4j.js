...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,addmentions,gapscan,callback) {
    ...
    if (concepts.length > 0) {
      ...
      for (var index = 1; index < concepts.length; ++ index) {
          ...
          if (concepts_added.indexOf(concepts[index]) == -1) {
              ...
              createNodesQuery += ' MERGE (concept:Concept ' + '{name:ConceptName}) ON CREATE SET concept.uid="' + node_uid + '" ';
    ...
    if (concepts.length > 0) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            // Link to user
            createNodesQuery += 'CREATE (concept)-[:BY {context:c_' + indx + '.uid,timestamp:s.timestamp,statement:s.uid}]->(u) ';

            // Link to context
            createNodesQuery += 'CREATE (concept)-[:OF {context:c_' + indx + '.uid,user:u.uid,timestamp:s.timestamp}]->(s)  CREATE (concept)-[:AT {user:u.uid,timestamp:s.timestamp,context:c_' + indx + '.uid,statement:s.uid}]->('+'c_' + indx + ') ';
        ...
        for (var cindx = 0; cindx < contexts.length; ++ cindx) {
                    ...
                    createNodesQuery += ', c_' + cindx;
        }

        createNodesQuery += ', s ';


        createNodesQuery += ' UNWIND $conceptsRelations as conceptsRelation ';
        ...
        if (concepts[minusOne] !== concepts[index]) {
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
              ...
              if ((mentions.length == 0) || (mentions.length > 0 && addmentions != 'link')) {
                ...
                params['conceptsRelations'].push['from'] = concepts[minusOne;
                params['conceptsRelations']['to'] = concepts[index];
                params['conceptsRelations']['context'] = concepts[index];
