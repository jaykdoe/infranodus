...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,addmentions,gapscan,callback) {
    ...
    for (var cindx = 0; cindx < contexts.length; ++ cindx) {
        ...
        createContexts += 'MERGE (' + 'c_' + cindx + ':Context ' + '{name:"' + contexts[cindx].name + '",by:u.uid,uid:"'+contexts[cindx].uid+'"}) ON CREATE SET ' + 'c_' + cindx + '.timestamp="' + timestamp + '" MERGE ' + '(c_' + cindx + ')' + '-[:BY{timestamp:"' + timestamp + '"}]->(u) ';
    }
    ...
    if (concepts.length > 0) {
        ...
        // Add to array to avoid repetitions and to be able to call convertName
        concepts_added.push(concepts[0]);

        createNodesQuery += 'MERGE (' + convertName(concepts[0], concepts_added) + ':Concept ' + '{name:"' + concepts[0] + '"}) ON CREATE SET ' + convertName(concepts[0], concepts_added) +  '.uid="' + node_uid + '" ';
    }
    ...
    if (concepts.length > 0) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            // Link to user
            createEdgesQuery += 'CREATE (' + convertName(concepts[0], concepts_added) + ')-[:BY {context:c_' + indx + '.uid,timestamp:s.timestamp,statement:s.uid}]->(u) ';

            // Link to context
            createEdgesQuery += 'CREATE (' + convertName(concepts[0], concepts_added) + ')-[:OF {context:c_' + indx + '.uid,user:u.uid,timestamp:s.timestamp}]->(s)  CREATE (' + convertName(concepts[0], concepts_added) + ')-[:AT {user:u.uid,timestamp:s.timestamp,context:c_' + indx + '.uid,statement:s.uid}]->('+'c_' + indx + ') ';
    ...
    if (concepts.length == 1) {
        ...
        if (mentions.length == 0) {
            ...
            for (var nindx = 0; nindx < contexts.length; ++ nindx) {
                ...
                createEdgesQuery += 'CREATE (' + convertName(concepts[0], concepts_added) +')-[:TO {context:c_' + nindx + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->(' + convertName(concepts[0], concepts_added) + ') ';
    ...
    for (var index = 1; index < concepts.length; ++ index) {
        ...
        if (concepts_added.indexOf(concepts[index]) == -1) {
            ...
            createNodesQuery += 'MERGE (' + convertName(concepts[index], concepts_added) + ':Concept ' + '{name:"' + concepts[index] + '"}) ON CREATE SET ' + convertName(concepts[index], concepts_added)  + '.uid="' + node_uid + '" ';
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            createEdgesQuery += 'CREATE (' + convertName(concepts[index], concepts_added)  + ')-[:BY {context:c_' + indx + '.uid,timestamp:"' +(timestamp+index)+ '",statement:s.uid}]->(u) ';
            createEdgesQuery += 'CREATE (' + convertName(concepts[index], concepts_added) + ')-[:OF {context:c_' + indx + '.uid,user:u.uid,timestamp:"'+(timestamp+index)+'"}]->(s) CREATE (' + convertName(concepts[index], concepts_added) + ')-[:AT {user:u.uid,timestamp:"' + (timestamp+index) + '",context:c_' + indx + '.uid,statement:s.uid}]->('+ 'c_' + indx +') ';
        }
