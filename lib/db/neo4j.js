...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,addmentions,gapscan,callback) {
    ...
    for (var cindx = 0; cindx < contexts.length; ++ cindx) {
        ...
        createContexts += 'MERGE (' + 'c_' + contexts[cindx].name + ':Context ' + '{name:"' + contexts[cindx].name + '",by:u.uid,uid:"'+contexts[cindx].uid+'"}) ON CREATE SET ' + 'c_' + contexts[cindx].name + '.timestamp="' + timestamp + '" MERGE ' + '(c_' + contexts[cindx].name + ')' + '-[:BY{timestamp:"' + timestamp + '"}]->(u) ';
    }
    ...
    if (concepts.length > 0) {
        createNodesQuery += 'MERGE (' + convertName(concepts[0]) + ':Concept ' + '{name:"' + concepts[0] + '"}) ON CREATE SET ' + convertName(concepts[0]) +  '.uid="' + node_uid + '" ';
    }
    ...
    if (typeof mentions !== 'undefined' && mentions.length > 0) {
        ...
        for (var puka = 0; puka < mentions.length; puka++) {
            ...
            if (mentions_added.indexOf(mentions[puka]) == -1) {
                ...
                createFirstMentionQuery += 'MERGE (' + convertMention(mentions[puka]) + ':Concept ' + '{name:"' + current_mention + '"}) ON CREATE SET ' + convertMention(mentions[puka])  + '.uid="' + ment_uid + '" ';
    ...
    if (concepts.length > 0) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            // Link to user
            createEdgesQuery += 'CREATE (' + convertName(concepts[0]) + ')-[:BY {context:c_' + contexts[indx].name + '.uid,timestamp:s.timestamp,statement:s.uid}]->(u) ';

            // Link to context
            createEdgesQuery += 'CREATE (' + convertName(concepts[0]) + ')-[:OF {context:c_' + contexts[indx].name + '.uid,user:u.uid,timestamp:s.timestamp}]->(s)  CREATE (' + convertName(concepts[0]) + ')-[:AT {user:u.uid,timestamp:s.timestamp,context:c_' + contexts[indx].name + '.uid,statement:s.uid}]->('+'c_' + contexts[indx].name+') ';
            ...
            if (typeof mentions !== 'undefined' && mentions.length > 0) {
                for (var index = 0; index < mentions.length; index++) {
                    ...
                    createMentionsQuery += 'CREATE (' + convertName(concepts[0]) +')-[:TO {context:c_' + contexts[indx].name + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edgement_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[index]) + ') ';
                }
            }
    ...
    if (concepts.length == 1) {
        ...
        if (mentions.length == 0) {
            ...
            for (var nindx = 0; nindx < contexts.length; ++ nindx) {
                ...
                createEdgesQuery += 'CREATE (' + convertName(concepts[0]) +')-[:TO {context:c_' + contexts[nindx].name + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->(' + convertName(concepts[0]) + ') ';
    ...
    if (mentions.length == 1) {
        ...
        if (concepts.length == 0) {
            ...
            for (var mindx = 0; mindx < contexts.length; ++ mindx) {
                ...
                createMentionsQuery += 'CREATE (' + convertMention(mentions[0]) +')-[:TO {context:c_' + contexts[mindx].name + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + ment_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[0]) + ') ';
    ...
    for (var index = 1; index < concepts.length; ++ index) {
        ...
        if (concepts_added.indexOf(concepts[index]) == -1) {
            ...
            createNodesQuery += 'MERGE (' + convertName(concepts[index]) + ':Concept ' + '{name:"' + concepts[index] + '"}) ON CREATE SET ' + convertName(concepts[index])  + '.uid="' + node_uid + '" ';
        ...
        if (concepts[minusOne] !== concepts[index]) {
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
              ...
              if ((mentions.length == 0) || (mentions.length > 0 && addmentions != 'link')) {
                ...
                createEdgesQuery += 'CREATE (' + convertName(concepts[minusOne]) +')-[:TO {context:c_' + contexts[indx].name + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->(' + convertName(concepts[index]) + ') ';
                ...
                if (typeof mentions !== 'undefined' && mentions.length > 0) {
                    for (var m = 0; m < mentions.length; m++) {
                        edgement_uid = uuid.v1();
                        createMentionsQuery += 'CREATE (' + convertName(concepts[index]) +')-[:TO {context:c_' + contexts[indx].name + '.uid,statement:s.uid,user:u.uid,timestamp:s.timestamp,uid:"' + edgement_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[m]) + ') ';
                    }
                }
            ...
            for (var indexGap = leftGap; indexGap < (index - 1); ++indexGap) {
                ...
                if (concepts[indexGap] !== concepts[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                        ...
                        // Make query
                        createEdgesQuery += 'CREATE (' + convertName(concepts[indexGap]) +')-[:TO {context:c_' + contexts[indx].name + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"' + scanGap + '",weight:"' + weightInGap + '"}]->(' + convertName(concepts[index]) + ') ';
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            createEdgesQuery += 'CREATE (' + convertName(concepts[index])  + ')-[:BY {context:c_' + contexts[indx].name + '.uid,timestamp:"' +(timestamp+index)+ '",statement:s.uid}]->(u) ';
            createEdgesQuery += 'CREATE (' + convertName(concepts[index]) + ')-[:OF {context:c_' + contexts[indx].name + '.uid,user:u.uid,timestamp:"'+(timestamp+index)+'"}]->(s) CREATE (' + convertName(concepts[index]) + ')-[:AT {user:u.uid,timestamp:"' + (timestamp+index) + '",context:c_' + contexts[indx].name + '.uid,statement:s.uid}]->('+ 'c_' + contexts[indx].name +') ';
        }
    ...
    if (mentions.length > 0) {
        ...
        for (var index = 0; index < mentions.length; index++) {
            ...
            for (var iter = 0; iter < index; iter++) {
                ...
                if (mentions[iter] !== mentions[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                        ...
                        createMentionsQuery += 'CREATE (' + convertMention(mentions[iter]) +')-[:TO {context:c_' + contexts[indx].name + '.uid,statement:s.uid,user:u.uid,timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->(' + convertMention(mentions[index]) + ') ';
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
                createMentionsQuery += 'CREATE (' + convertMention(mentions[index])  + ')-[:BY {context:c_' + contexts[indx].name + '.uid,timestamp:"' +(timestamp+index)+ '",statement:s.uid}]->(u) ';
                createMentionsQuery += 'CREATE (' + convertMention(mentions[index]) + ')-[:OF {context:c_' + contexts[indx].name + '.uid,user:u.uid,timestamp:"'+(timestamp+index)+'"}]->(s) CREATE (' + convertMention(mentions[index]) + ')-[:AT {user:u.uid,timestamp:"' + (timestamp+index) + '",context:c_' + contexts[indx].name + '.uid,statement:s.uid}]->('+ 'c_' + contexts[indx].name +') ';
            }
    ...
    for (var indx = 0; indx < contexts.length; ++ indx) {
        createStatement += 'CREATE (s)-[:BY {context:c_' + contexts[indx].name + '.uid,timestamp:s.timestamp}]->(u) ';
        createStatement += 'CREATE (s)-[:IN {user:u.id,timestamp:s.timestamp}]->('+'c_' + contexts[indx].name+') ';
    }
