                        ...
                        if (typeof mentions !== 'undefined' && mentions.length > 0) {
                              ...
                              for (var m = 0; m < mentions.length; m++) {
                                if (addmentions == 'remove' || addmentions == 'link') {
                                  params['statements'][sindex]['mentionsRelations'].push({from:concepts[index],to:mentions[m].substring(1),context:contexts[indx].uid,statement:params['statements'][sindex]['uid'],user:user.uid,timestamp:timestamp+index,uid:'apoc.create.uuid()',gapscan:"1",weight:narrativeScanWeight});
                                }
                                else {
