...
User.modifySettings = function (user_id, fullscan, fullview, morphemes, hashnodes, maxnodes, inlanguage, palette, background, midi, voice_continues, label_threshold, topnodes, mentions, customization, stopwords, callback) {
    ...
    var modify_query = 'MATCH (u:User{uid:"' + user_id + '"}) ' +
        ...
        'u.inlanguage = "' + inlanguage + '", ' +
        'u.palette = "' + palette + '", ' +
        'u.background = "' + background + '", ' +
        'u.midi = "' + midi + '", ' +
        'u.voice_continues = "' + voice_continues + '", ' +
        'u.label_threshold = "' + label_threshold + '", ' +
        'u.topnodes = "' + topnodes + '", ' +
        'u.mentions = "' + mentions + '", ' +
        'u.customization = "' + customization + '", ' +
        'u.stopwords = "' + stopwords + '";';
...
User.checkHostedPage = function (hosted_page, callback) {
  if (config.chargebee && config.chargebee.site && config.chargebee.api_key) {
    ...
    chargebee.configure({site : chargebee_site,
      api_key : chargebee_apikey});
    chargebee.hosted_page.retrieve(hosted_page_id).request(
      function(error,result){
        if(error){
          //handle error
          callback(error)
