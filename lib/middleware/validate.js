...
var tokenizerEN = new natural.AggressiveTokenizer();
var tokenizerSV = new natural.AggressiveTokenizerSv();
var tokenizerFR = new natural.AggressiveTokenizerFr();
var tokenizerDE = new natural.WordTokenizer();
var tokenizerRU = new natural.AggressiveTokenizerRu();
var tokenizerES = new natural.AggressiveTokenizerEs();
var tokenizerPT = new natural.AggressiveTokenizerPt();
...
function extractConcepts(statement, morphemes, hashnodes, stopwords_custom, res) {
    ...
    if (lng == 'english' || lng == 'penglish') {

      concepts = tokenizerEN.tokenize(statement.toLowerCase());

    }
    else if (lng == 'russian') {

      concepts = tokenizerRU.tokenize(statement.toLowerCase());

    }
    else if (lng == 'french') {

      concepts = tokenizerFR.tokenize(statement.toLowerCase());

    }
    else if (lng == 'pswedish' || lng == 'swedish') {
        concepts = tokenizerSV.tokenize(statement.toLowerCase());
    }

    else if (lng == 'pspanish' || lng == 'spanish') {
        concepts = tokenizerES.tokenize(statement.toLowerCase());
    }

    else if (lng == 'pportugese' || lng == 'portugese') {
        concepts = tokenizerPT.tokenize(statement.toLowerCase());
    }

    else if (lng == 'german') {
      var conceptsRaw = statement.toLowerCase().replace(/[^a-zA-Z0-9\u00C0-\u00FF]+/g, ' ').split(' ');
    ...
    for (var i = 0; i < concepts.length; i++) {
        if ((stopwords.indexOf(concepts[i]) == -1  && concepts[i].length > 0) || hashtags.indexOf(concepts[i]) > -1) {
     ...
     // this lemmatizer yields several results so we need to take only the first one
      ...
      for (var i = 0; i < lemmaspre.length; ++i) {
          ...
          if (lemmaspre[i]['lemma'] != undefined && lemmaspre[i]['id'] == lemmacounter) {
            if ((stopwords.indexOf(lemmaspre[i]['lemma'].toLowerCase()) == -1  && lemmaspre[i]['lemma'].toLowerCase().length > 0) || hashtags.indexOf(lemmaspre[i]['lemma'].toLowerCase()) > -1) {
    ...
    // It is not French language?
      ...
      for (var i = 0; i < lemmas.length; ++i) {
          ...
          if (lemmas[i][0] != undefined) {
              ...
              if ((stopwords.indexOf(lemmas[i][0]) == -1  && lemmas[i][0].length > 0) || hashtags.indexOf(lemmas[i][0]) > -1) {
...
exports.sanitize = function(statement){
        ...
        if (result) {
          ...
          result = result.replace(/[„“]/g, '"').replace(/[«»]/g, '"').replace(/\u0000/g, '0');
...
exports.getContextsList = function(flag){
    ...
    return function(req, res, next){
            ...
            if (res.locals.user) {
                ...
                if ((res.locals.user.name != req.params.user) && (req.params.user != undefined)) {
                    contextforuser = req.params.user;
                    flag = 'public';
            ...
            var query = 'MATCH (u:User{name:"' + contextforuser + '"}), (ctx:Context), (ctx)-[:BY]->(u) WITH DISTINCT ctx MATCH (s:Statement), (ctx)<-[rel:IN]-(s) ' + querymod + ' RETURN DISTINCT ctx.name, ctx.uid;'
...
exports.getUserID = function(){
    ...
    return function(req, res, next){
        ...
        if (req.params.user) {
            ...
            var query = 'MATCH (u:User{name:"' + userid + '"}) RETURN u.uid, u.stopwords, u.topnodes, u.palette, u.background, u.label_threshold, u.maxnodes';
