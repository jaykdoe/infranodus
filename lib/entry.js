...
Entry.prototype.savetrans = function(fn){
    ...
    CypherQuery.addStatement(user, statements, contexts, addmentions, gapscan, function(cypherRequest) {
...
Entry.getNodes = function(receiver, perceiver, contexts, fullview, showcontexts, res, req, fn){
    ...
    if (contexts.length > 0 && contexts[0]) {
        ...
        for (var u = 0; u < (contexts.length); ++u) {
          ...
          querynodes += " apoc.index.relationships('TO','context:" + Instruments.findInArray(contexts_map, contexts[u])  + "') ";
          querynodes += " YIELD rel, start, end " + view_filter +
          ...
          context_query += " rel.context='" + Instruments.findInArray(contexts_map, contexts[u]) + "' ";
    ...
    // This is when we view all the (publicly) available graphs of a person...
        ...
        if (contexts_map.length <= 3) {
          for (var c = 0; c < contexts_map.length; c++) {
            ...
            querynodes += " apoc.index.relationships('TO','context:" + contexts_map[c][1] + "') ";
            querynodes += " YIELD rel, start, end " + view_filter +
                          " WITH DISTINCT rel, start, end " +
                          " RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                          " end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
        ...
        // We have more than 3 contexts, so the query will be a bit long, therefore we design it differently
          ...
          if (((receiver == perceiver) && (receiver !== '')) || ((receiver !== '') && (perceiver === ''))) {
              ...
              querynodes = "CALL apoc.index.relationships('TO','context:(" + filter_context + ")') " +
                           "YIELD rel, start, end " + view_filter +
                           "WITH DISTINCT rel, start, end WHERE rel.user='" + receiver + "' " +
                           "RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                           "end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
                           "rel.context AS context_id, rel.statement AS statement_id, rel.weight AS weight ";
              ...
              querynodes =  "CALL apoc.index.relationships('TO','context:(" + filter_context + ")') " +
                            "YIELD rel, start, end " + view_filter +
                            "WITH DISTINCT rel, start, end WHERE rel.user='" + perceiver + "' " +
                            "RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                            "end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
                            "rel.context AS context_id, rel.statement AS statement_id, rel.weight AS weight ";
        ...
        if (showcontexts) {
          if (((receiver == perceiver) && (receiver !== '')) || ((receiver !== '') && (perceiver === ''))) {
             ...
             show_contexts =        " UNION CALL apoc.index.relationships('AT','user:" + receiver + "') " +
                                    "YIELD rel, start, end " + view_filter +
                                    "WITH DISTINCT rel, start, end " + context_query +
                                    "RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                                    "end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
                                    "rel.context AS context_id, rel.statement AS statement_id, rel.weight AS weight;";
         }
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            ...
            if ((sourcein) && (targetin)) {
                ...
                if (edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]] && edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]].edge_context == Instruments.findInArray(contexts_map, nodes_object[i][5], 1)) {
                  edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]].statement_id.push(nodes_object[i][6]);
                  ...
                  edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]] = {
                    ...
                    id: nodes_object[i][4],
                    edge_context: Instruments.findInArray(contexts_map, nodes_object[i][5], 1),
