...
Entry.getConnectedContexts = function(receiver, perceiver, keywords, fn){
     ...
     if (keywords.length > 0 && keywords[0]) {
        ...
        keywords_query = "['" + keywords[0] + "']";

         for (var u=1;u<(keywords.length);++u) {
            keywords_query += ",['" + keywords[u] + "']";
         }
    ...
    // Receiver - the one who's watching it
     ...
     if (((receiver == perceiver) && (receiver !== '')) || ((receiver !== '') && (perceiver === ''))) {
         ...
         var conContextQuery = "MATCH (c1:Concept),(c2:Concept),(c1)--(c2) " +
                               "WHERE  c1.name in (" + keywords + ") AND c2.name in (" + keywords + ") " +
...
Entry.getNodes = function(receiver, perceiver, contexts, fullview, showcontexts, res, req, fn){
   ...
   /* var querynodes =    "MATCH " +
        ...
        // TODO make it possible to extract stopwords that were selected by the user
