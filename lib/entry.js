...
Entry.getNodes = function(receiver, perceiver, contexts, fullview, showcontexts, res, req, fn){
    ...
    if (contexts.length > 0 && contexts[0]) {
        ...
        querynodes += " apoc.index.relationships('TO','context:" + Instruments.findInArray(contexts_map, contexts[0])  + "') ";
        ...
        for (var u = 1; u < (contexts.length); ++u) {
        querynodes += " UNION CALL apoc.index.relationships('TO','context:" + Instruments.findInArray(contexts_map, contexts[u])  + "') ";
        querynodes += " YIELD rel, start, end " + view_filter +
                      " WITH DISTINCT rel, start, end " +
                      " RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                      " end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
    ...
    // This is when we view all the (publicly) available graphs of a person...
        ...
        // We have more than 3 contexts, so the query will be a bit long, therefore we design it differently
          ...
          if (((receiver == perceiver) && (receiver !== '')) || ((receiver !== '') && (perceiver === ''))) {
              ...
              querynodes = "CALL apoc.index.relationships('TO','user:" + receiver + "') " +
                           "YIELD rel, start, end " + view_filter +
                           "WITH DISTINCT rel, start, end " +
                           "RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                           "end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
                           "rel.context AS context_id, rel.statement AS statement_id, rel.weight AS weight " + show_contexts;
              ...
              // TODO this can be optimized if we don't use the context query match but instead filter the rels by the visible contexts maybe?

              querynodes =  "CALL apoc.index.relationships('TO','user:" + perceiver + "') " +
                            "YIELD rel, start, end " + view_filter +
                            "WITH DISTINCT rel, start, end " + context_query +
                            "RETURN DISTINCT start.uid AS source_id, start.name AS source_name, " +
                            "end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, " +
