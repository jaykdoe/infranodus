...
Entry.getNodes = function(receiver, perceiver, contexts, fullview, showcontexts, res, req, fn){
    ...
    var contexts_map = res.locals.contextslist;
    ...
    if (contexts.length > 0 && contexts[0]) {
        ...
        context_query = 'WHERE (rel.context="' + Instruments.findInArray(contexts_map, contexts[0]) + '" ';
        for (var u = 1; u < (contexts.length); ++u) {
            context_query += 'OR rel.context="' + Instruments.findInArray(contexts_map, contexts[u]) + '" ';
        }
   ...
   /* var querynodes =    "MATCH " +
        ...
        if (context_query.length > 0) {
          context_query = "MATCH (ctx:Context) " + context_query + " AND ctx.public = '1' AND ctx.uid = rel.context ";
        }
        else {
